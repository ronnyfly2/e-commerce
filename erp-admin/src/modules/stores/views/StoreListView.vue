...
async function load() {
  ...
  try {
    const res = await storeApi.getAll({ page: page.value, search: search.value });
...
async function confirmDelete() {
  ...
  try {
    await storeApi.remove(deleteTarget.value);
...
<template>
  <div>
    <PageHeader title="Tiendas" description="Gestiona los puntos de venta">
      <BaseButton @click="router.push('/stores/new')"><PlusIcon class="h-4 w-4" /> Nueva tienda</BaseButton>
    </PageHeader>
    ...
    <div v-if="!error" class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
      ...
      <template v-else>
        <EmptyState v-if="list.length === 0">
          <BaseButton @click="router.push('/stores/new')"><PlusIcon class="h-4 w-4" /> Crear primera tienda</BaseButton>
        </EmptyState>
        <table v-else class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead class="bg-gray-50 dark:bg-gray-800">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Sucursal</th>
              <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Estado</th>
              <th class="px-6 py-3" />
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="s in list" :key="s.id" class="hover:bg-gray-50 dark:hover:bg-gray-800">
              <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{{ s.name }}</td>
              <td class="whitespace-nowrap px-6 py-4 font-mono text-sm text-gray-600 dark:text-gray-400">{{ s.code }}</td>
              <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{{ s.branch?.name ?? '—' }}</td>
              <td class="whitespace-nowrap px-6 py-4">
                <BaseBadge :variant="s.isActive ? 'success' : 'danger'">{{ s.isActive ? 'Activa' : 'Inactiva' }}</BaseBadge>
              </td>
              <td class="whitespace-nowrap px-6 py-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  <button type="button" class="rounded p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-gray-700" @click="router.push(`/stores/${s.id}/edit`)"><PencilIcon class="h-4 w-4" /></button>
                  <button type="button" class="rounded p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20" @click="deleteTarget = s.id"><TrashIcon class="h-4 w-4" /></button>
                </div>
              </td>
            </tr>
          </tbody>
    ...
    <BaseModal :open="!!deleteTarget" title="Eliminar tienda" @close="deleteTarget = null">
      <p class="text-sm text-gray-600 dark:text-gray-400">¿Eliminar esta tienda?</p>
      <template #footer>
