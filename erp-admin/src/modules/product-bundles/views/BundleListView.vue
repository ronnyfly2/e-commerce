...
async function load() {
  ...
  try {
    const res = await productBundleApi.getAll({ page: page.value, search: search.value });
...
async function confirmDelete() {
  ...
  try {
    await productBundleApi.remove(deleteTarget.value);
...
<template>
  <div>
    <PageHeader title="Paquetes" description="Combos de varios productos a un precio fijo, para ventas al por mayor">
      <BaseButton @click="router.push('/bundles/new')"><PlusIcon class="h-4 w-4" /> Nuevo paquete</BaseButton>
    </PageHeader>
    ...
    <div v-if="!error" class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
      ...
      <template v-else>
        <EmptyState v-if="list.length === 0">
          <BaseButton @click="router.push('/bundles/new')"><PlusIcon class="h-4 w-4" /> Crear primer paquete</BaseButton>
        </EmptyState>
        <table v-else class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="b in list" :key="b.id" class="hover:bg-gray-50 dark:hover:bg-gray-800">
              ...
              <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{{ b.items.length }} producto(s)</td>
              <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{{ Number(b.price).toFixed(2) }}</td>
              <td class="whitespace-nowrap px-6 py-4">
                <BaseBadge :variant="b.isActive ? 'success' : 'danger'">{{ b.isActive ? 'Activo' : 'Inactivo' }}</BaseBadge>
              </td>
              <td class="whitespace-nowrap px-6 py-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  <button type="button" class="rounded p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-gray-700" @click="router.push(`/bundles/${b.id}/edit`)"><PencilIcon class="h-4 w-4" /></button>
    ...
    <BaseModal :open="!!deleteTarget" title="Eliminar paquete" @close="deleteTarget = null">
      <p class="text-sm text-gray-600 dark:text-gray-400">¿Eliminar este paquete?</p>
      <template #footer>
