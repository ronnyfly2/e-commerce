...
async function load() {
  ...
  try {
    const res = await roleApi.getAll({ page: page.value, search: search.value });
...
async function confirmDelete() {
  ...
  try {
    await roleApi.remove(deleteTarget.value);
...
<template>
  <div>
    <PageHeader title="Roles" description="Define los roles y permisos del sistema">
      <BaseButton @click="router.push('/roles/new')"><PlusIcon class="h-4 w-4" /> Nuevo rol</BaseButton>
    </PageHeader>
    ...
    <div v-if="!error" class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
      ...
      <template v-else>
        <EmptyState v-if="list.length === 0">
          <BaseButton @click="router.push('/roles/new')"><PlusIcon class="h-4 w-4" /> Crear primer rol</BaseButton>
        </EmptyState>
        <table v-else class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead class="bg-gray-50 dark:bg-gray-800">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Permisos</th>
              <th class="px-6 py-3" />
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="role in list" :key="role.id" class="hover:bg-gray-50 dark:hover:bg-gray-800">
              <td class="whitespace-nowrap px-6 py-4">
                <div class="flex items-center gap-2">
                  <LockClosedIcon v-if="role.isSystem" class="h-4 w-4 text-gray-400" />
              ...
              <td class="whitespace-nowrap px-6 py-4">
                <BaseBadge :variant="role.isSystem ? 'info' : 'default'">{{ role.isSystem ? 'Sistema' : 'Personalizado' }}</BaseBadge>
              </td>
              <td class="px-6 py-4 text-sm text-gray-500">{{ role.permissions.length }} permiso(s)</td>
              <td class="whitespace-nowrap px-6 py-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  <button v-if="!role.isSystem" type="button" class="rounded p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-gray-700" @click="router.push(`/roles/${role.id}/edit`)"><PencilIcon class="h-4 w-4" /></button>
                  <button v-if="!role.isSystem" type="button" class="rounded p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20" @click="deleteTarget = role.id"><TrashIcon class="h-4 w-4" /></button>
                </div>
              </td>
            </tr>
          </tbody>
    ...
    <BaseModal :open="!!deleteTarget" title="Eliminar rol" @close="deleteTarget = null">
      <p class="text-sm text-gray-600 dark:text-gray-400">¿Eliminar este rol? Los usuarios asignados quedarán sin rol.</p>
      <template #footer>
